...
export const initRoom$ = createEffect(
  (actions$ = inject(Actions), chatApiService = inject(ChatApiService), router = inject(Router)) => {
    return actions$.pipe(
      ofType(CHAT_ACTIONS.initRoom),
      exhaustMap(({ userId }) =>
        chatApiService.initRoom(userId).pipe(
...
export const getRooms$ = createEffect(
  (actions$ = inject(Actions), chatApiService = inject(ChatApiService)) => {
    return actions$.pipe(
      ofType(CHAT_ACTIONS.getRooms),
      exhaustMap(() => {
        return chatApiService.getRooms().pipe(
...
export const initChatListener$ = createEffect(
  (actions$ = inject(Actions), chatSocketService = inject(ChatSocketService)) => {
    return actions$.pipe(
...
export const enterTheRoom$ = createEffect(
  (actions$ = inject(Actions), chatSocketService = inject(ChatSocketService)) => {
    return actions$.pipe(
      ...
      switchMap((message) => [CHAT_ACTIONS.messageReceived({ message })]),
      catchError((error) => of(CHAT_ACTIONS.error({ error }))),
    );
  },
  { functional: true },
    ...
    return actions$.pipe(
      ...
      switchMap(({ roomId }) => {
        return chatApiService.getMessages(roomId).pipe(
          switchMap((messages) => [
            CHAT_ACTIONS.setMessages({ messages }),
            new FocusAction(`${CHAT_FORMS_IDS.message}.text`),
          ]),
          catchError((error) => of(CHAT_ACTIONS.error({ error }))),
        );
      }),
...
export const sendMessage$ = createEffect(
  (actions$ = inject(Actions), store$ = inject(Store<AppState>), chatSocketService = inject(ChatSocketService)) => {
    return actions$.pipe(
      ...
      switchMap(([, form]) => {
        ...
        return [
          new SetValueAction(CHAT_FORMS_IDS.message, { ...form?.value, text: '' }),
          new FocusAction(`${CHAT_FORMS_IDS.message}.text`),
        ];
      }),
      catchError((error) => of(CHAT_ACTIONS.error({ error }))),
